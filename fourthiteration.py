...
# Variable to track the first obstacle after restart
...
def add_new_obstacle():
    ...
    # Randomly decide if the obstacle starts from the floor or the ceiling
    if random.choice([True, False]):
        obstacle_y = height - obstacle_height - grass_height  # From the floor
    else:
        obstacle_y = 0  # From the ceiling
...
while True:
    ...
    if first_obstacle_after_restart and current_time - last_obstacle_time > 3:
        # Add the first obstacle after the 3-second delay
        add_new_obstacle()
        ...
        # Add subsequent obstacles based on the regular generation interval
        add_new_obstacle()
        last_obstacle_time = current_time
