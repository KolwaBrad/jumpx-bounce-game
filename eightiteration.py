...
# Star properties
star_radius = 30
star_size_increase = 0.9  # Increase by 90%
...
# Variables to handle ball size change
ball_size_change_start_time = 0
original_bird_radius = bird_radius
...
def handle_star_collision():
    global ball_size_change_start_time, bird_color, bird_radius

    if bird.colliderect(star_rect):
        ball_size_change_start_time = time.time()
        bird_color = star_color

def reset_ball_size():
    global bird_radius, ball_size_change_start_time
...
while True:
    ...
    if ball_size_change_start_time > 0 and current_time - ball_size_change_start_time <= star_generation_interval:
        bird_radius += int(original_bird_radius * star_size_increase)
    ...
    pygame.draw.circle(screen, star_color, (star_x, star_y), int(star_radius * (1 + star_size_increase)))
