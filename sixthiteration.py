...
# Variables to handle ball property changes
ball_property_change_duration = 5
ball_property_change_start_time = 0
...
def handle_star_collision():
    global gravity, obstacle_speed, ball_property_change_start_time

    if bird.colliderect(star_rect):
        ball_property_change_start_time = time.time()

        if star_color == (255, 255, 0):  # Yellow
            bird_color = (255, 255, 0)  # Change ball color to yellow
            bird.inflate_ip(bird_radius * 2, bird_radius * 2)  # Double the ball size
        elif star_color == (255, 182, 193):  # Pink
            bird_color = (255, 182, 193)  # Change ball color to pink
            gravity /= 2  # Slow down gravity
            obstacle_speed /= 2  # Slow down obstacle speed
        elif star_color == (0, 0, 0):  # Black
            bird_color = (0, 0, 0)  # Change ball color to black
            gravity *= 2  # Double gravity
            obstacle_speed *= 2  # Double obstacle speed

def reset_ball_properties():
    global gravity, obstacle_speed, ball_property_change_start_time

    gravity = original_gravity
    obstacle_speed = original_speed
    ball_property_change_start_time = 0
...
while True:
    ...
    if ball_property_change_start_time > 0 and current_time - ball_property_change_start_time > ball_property_change_duration:
        reset_ball_properties()
